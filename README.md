...
содержаться в python-файле **config.py**, общие настройки, которые в будущем стоит перенести
в интерфейс содержаться в **settings.py**.
...
Модуле **earth.py** содержит методы формирования URL-запросов для построения растровых статических
...
названия которых говорят сами за себя, а также оригинальная иконка для приложения **icon.ico**,
файл необходимых сторонних модулей виртуального окружения **requirements.txt** и в формате
*markdown* настоящая инструкция **README.md**. Отдельно стоит отметить, что в корневой папке
...
использования настоящего пакета программ, необходимо получить самостоятельно. Данные из
**tsukanoff.py** импортируются только в модуле **config.py**, где в комментариях даны
...
После загрузки файла с проектными точками, программа сформирует запрос и получит сформированную
растровую карту Яндекс с отмеченными на ней точками из загруженной таблицы. Пример для нашего
проекта представлен на скриншоте ***Рис.3***.
...
Это описание будет доступно группе полевых специалистов.
...
### СКРИНШОТ 2 - Клавиша "Открыть задание" демонстрирует растровую карту с плановыми точками, которую мы видели в основном приложении, описание задания и название проекта.
...
### СКРИНШОТ 4 - Кнопка "Проверить свою геолокацию". Запрос подтверждения отправки геолокации боту. 

![05](https://github.com/user-attachments/assets/02f04d73-b1a0-4096-9823-953d85e88534)
### СКРИНШОТ 5 - Получение своих координат и *инлайн-клавиатура* с выбором web-карт.

![06](https://github.com/user-attachments/assets/1915f614-520e-4768-a0b7-ef40ad8984e4)
### СКРИНШОТ 6 - Пример перехода на *OpenStreetMap* сервис.
...
метеорологии, экологии, геофизики и других.
